...
echo "Compiling glib"
mkdir ${DEPS}/glib
curl -k -Ls https://ftp.acc.umu.se/pub/gnome/sources/glib/2.40/glib-${VERSION_GLIB}.tar.xz | xz -d | tar -xC ${DEPS}/glib --strip-components=1
echo "Fetching glib"
# curl -o /tmp/glib-2.40.2.tar.xz https://workserver.cube.sg/tmp/glib-2.40.2.tar.xz
# curl -Ls https://workserver.cube.sg/tmp/glib--2.40.2.tar.xz | xz -d | tar -xC ${DEPS}/glib --strip-components=1
# tar xvf /tmp/glib-2.40.2.tar.xz -C ${DEPS}/glib --strip-components=1
...
mkdir ${DEPS}/nasm
curl -k -Ls https://www.nasm.us/pub/nasm/releasebuilds/${VERSION_NASM}/nasm-${VERSION_NASM}.tar.xz | xz -d | tar xC ${DEPS}/nasm --strip-components=1
cd ${DEPS}/nasm
...
mkdir ${DEPS}/orc
curl -k -Ls https://gstreamer.freedesktop.org/data/src/orc/orc-${VERSION_ORC}.tar.xz | xz -d | tar xC ${DEPS}/orc --strip-components=1
cd ${DEPS}/orc
./configure --prefix=${TARGET} --disable-shared --enable-static
make install
...
mkdir ${DEPS}/vips
curl -k -Ls https://github.com/jcupitt/libvips/archive/v${VERSION_VIPS}/vips-${VERSION_VIPS}.tar.gz | tar xzC ${DEPS}/vips --strip-components=1
cd ${DEPS}/vips
./autogen.sh --prefix=${TARGET} --disable-shared --enable-static --without-radiance --without-analyze
cp /io/vips-Makefile ${DEPS}/vips/Makefile
make install
